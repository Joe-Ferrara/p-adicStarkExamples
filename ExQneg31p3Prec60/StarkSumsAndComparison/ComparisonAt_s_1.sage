...
LValuesPos1 = [LValuesAt_s_1[2], LValuesAt_s_1[1]] ## Pos1 refers to alpha = 1 (p-stabilization)

LValuesNeg1 = [LValuesAt_s_1[5], LValuesAt_s_1[4]] ## Neg1 refers to alpha = -1 (p-stabilization)
...
############################################################
for k in range(0, 2):
##    ratioPos1 = (LValuesPos1[k])/(StSumsNeg1Try1[1 - k])
    ratioNeg1 = (LValuesNeg1[k])/(StSumsPos1Try1[1 - k])
...
for k in range(0, 2):
    ratioPos1 = (LValuesPos1[k])/(StSumsNeg1Try2[1 - k])
    ratioNeg1 = (LValuesNeg1[k])/(StSumsPos1Try2[1 - k])
...
GaussSumRatios = [GaussSums[0]/GaussSums[1]]

## approximate the GaussSumRatio
data = [GaussSumRatios[0].valuation(), GaussSumRatios[0].expansion()]
GaussSumRatioApprox = approximate(make_algebraic(data))

##valuationsPos1Try1 = []
valuationsNeg1Try1 = []
for i in range(0, len(RatioComparisonsNeg1Try1)):
