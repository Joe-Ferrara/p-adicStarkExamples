...
def unimod_matrices(r, s):
	...
	if s != 0:
		...
		v = [Matrix(2, 2,[1, list[0].numerator(),0,list[0].denominator()])]
		sig = -1
		for j in range(0,len(list)-1):
			...
			v.append(Matrix(2, 2, [sig * a, b, sig * c, d]))
			sig = -sig
		return v
	else:
		return []
...
def unimod_matrices2(r, s):
	...
	if s != 0:
		list = convergents(r/s)
		v = [Matrix(2, 2,[-list[0].numerator(), 1, -list[0].denominator(), 0])]
		sig = -1
		for j in range(0,len(list)-1):
			a=list[j].numerator()
			c=list[j].denominator()
			b=list[j+1].numerator()
			d=list[j+1].denominator()
			v.append(Matrix(2, 2, [-b, sig * a, -d, sig * c]))
			sig = -sig
		return v
	else:
		return []
...
def prep_hecke_individual(ell,N,M,m):
	...
	for a in srange(ell):
		...
		for b in range(len(v)):
			...
			j = M.P1_to_mats[i]
			B = M.mats[j]
			C = ~A#A.__invert__unit()
			gaminv = B * C
			ans[j] = ans[j] + [gaminv * gama]
	if N % ell != 0:
		gama=Matrix(2, 2, [ell, 0, 0, 1])
		t = gama * M.mats[M.gens[m]]
		v = unimod_matrices2(t[0,0], t[1,0]) + unimod_matrices(t[0,1], t[1,1])
		for b in range(len(v)):
			A = v[b]
