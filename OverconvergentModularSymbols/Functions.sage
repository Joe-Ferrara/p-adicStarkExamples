...
## NOTE: THIS DOES NOT ALWAYS WORK. WHEN IT DOES NOT WORK, MUST FIDDLE TO GET HECKE EIGENSYMBOL
...
## NOTE: This function will only work for the specific example Qsqrt17. Seems to work for the example Qneg31 as well
...
def cosetIntegralsAt_s_0(phi, ap, n):
    ## phi is the overconvergent modular symbol
    ## ap is the Up eigenvalue of phi
    ## n is the exponent of p in the sense that we want the cosets of p^nZ_p in Z_p
    p = phi.p()
    M = phi.num_moments()
    ans = []
    for i in range(0, p^n):
