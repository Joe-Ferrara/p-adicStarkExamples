## does the comparison at s=0 and gets what we predict


StLogsPos1Pre = load("pAdicLogsPos1Proj")
StLogsNeg1Pre = load("pAdicLogsNeg1Proj")
...
LValuesAt_s_0_Data = load("LValuesDataAt_s_0")
...
LValuesPos1 = [] ## Pos1 refers to alpha = 1 (p-stabilization)
for i in range(1, 5):
    LValuesPos1.append(LValuesAt_s_0[i])

LValuesNeg1 = [] ## Neg1 refers to alpha = -1 (p-stabilization)
for i in range(6, 10):
    LValuesNeg1.append(LValuesAt_s_0[i])
...
for k in range(0, 4):
    ratioPos1 = (LValuesPos1[k])/(StSumsPos1[3 - k])
    ratioNeg1 = (LValuesNeg1[k])/(StSumsNeg1[3 - k])
...
approx1Neg1 = []
for ratio in RatioComparisonsNeg1:
    data = [ratio.valuation(), ratio.expansion()]
    alg = make_algebraic(data)
    approx = approximate(alg)
